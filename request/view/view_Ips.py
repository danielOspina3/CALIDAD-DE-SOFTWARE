...
def getEPS(request):
    try:
        listIps = Ips.objects.all().order_by('nombre_ips')
...
def createEPS(request):
    try:
        ...
        if Ips.objects.filter(cod_ips=cod_ips).exists():
            logger.error("Error El codigo IPS ya esta en uso: " + str(e))
            return Response({"error": "El código IPS ya está en uso."}, status=status.HTTP_400_INTERNAL_SERVER_ERROR)
    ...
    except Ips.cod_ips.Validator.ValidationError as e:
        logger.error("Error de validacion al crear EPS: " + str(e))
        return Response({"error": "Error de validacion al crear EPS"}, status=status.HTTP_204_NO_CONTENT)
    except Exception as e:
